  ...
  for my $q ('', '"') {
    ...
    for my $i (0 .. @$ident_obj-1) {
      ...
      for my $c (0 .. @$concat_obj-1) {
        ...
        # plan when we have all of our test objects
        #$planned++ || plan tests => 2 * 2 * @$ident_obj * @$concat_obj * (@$select_obj+1) * 2 + 1;
        $planned++ || plan tests => 2 * 2 * @$ident_obj * (@$concat_obj-1) * (@$select_obj+1) * 2 + 1;  # remove TODO tests from above
